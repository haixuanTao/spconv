...
from spconv.constants import (
    ALL_WEIGHT_IS_KRSC,
    FILTER_HWIO,
    SPCONV_FX_TRACE_MODE,  # noqa
)
from spconv.core import ConvAlgo  # noqa
...
class SparseMaxPoolTestTorch(nn.Module):
    def __init__(
        self, num_layers, ndim, shape, kernel_size, stride, padding, dilation, algo
    ):
        super().__init__()
        self.algo = algo
        layers = [
        ...
        for i in range(1, num_layers):
            layers.append(
                spconv.SparseMaxPool3d(
                    kernel_size, stride, padding, dilation, algo=algo
                )
            )
        self.net = spconv.SparseSequential(
            *layers,
...
def test_spconv3d():
    ...
    for dev, shape, bs, IC, OC, k, s, p, d, al in params_grid(
        ...
        dtype = torch.float32
        net = (
            SparseConv3dTestTorch(1, 3, shape, IC, OC, k, s, p, d, algo=al)
            .to(device)
            .to(dtype)
        )
        net_ref = Conv3dTestTorch(1, 3, shape, IC, OC, k, s, p, d).to(device).to(dtype)
        ...
        if net.algo == ConvAlgo.Native and not ALL_WEIGHT_IS_KRSC:
            if FILTER_HWIO:
                filters = np.random.uniform(-1, 1, size=[k, k, k, IC, OC]).astype(
                    np.float32
                )
            else:
                filters = np.random.uniform(-1, 1, size=[k, k, k, OC, IC]).astype(
                    np.float32
                )
            ...
            filters = np.random.uniform(-1, 1, size=[OC, k, k, k, IC]).astype(
                np.float32
            )
            filters_t = torch.from_numpy(filters).to(device).to(dtype)
            net_ref.net[0].weight.data[:] = filters_t.permute(
...
def test_spdeconv3d():
    ...
    for dev, shape, bs, IC, OC, k, s, p, d, al in params_grid(
        ...
        features_dense = sparse_dict["features_dense"].astype(np.float32)
        net = SparseDeConv3dTestTorch(1, 3, shape, IC, OC, k, s, p, d, al).to(device)
