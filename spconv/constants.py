...
EDITABLE_INSTALLED = project_is_installed(PACKAGE_NAME) and project_is_editable(
    PACKAGE_NAME
)
...
if SAVED_WEIGHT_LAYOUT != "":
    assert SAVED_WEIGHT_LAYOUT in ["KRSC", "RSKC", "RSCK"], (
        "please set SAVED_WEIGHT_LAYOUT to KRSC, RSKC or RSCK"
    )
...
if _BOOST_ROOT is None:
    ...
    assert (BOOST_ROOT / "boost" / "geometry").exists(), (
...
SPCONV_BWD_SPLITK = list(
    map(int, os.getenv("SPCONV_BWD_SPLITK", "1,2,4,8,16,32,64").split(","))
)
