...
from spconv.pytorch.core import ImplicitGemmIndiceData, IndiceData, expand_nd
...
class SparseMaxPool(SparseModule):
    def __init__(
        ...
        if kv > 128:
            assert algo == ConvAlgo.Native, (
                "implicit gemm don't support kv >= 32 for now"
            )
        if CPU_ONLY_BUILD:
            print("cpu only build only support native algorithm")
    ...
    def forward(self, input: spconv.SparseConvTensor):
        ...
        if CPU_ONLY_BUILD:
            input._timer = None
        if is_int8:
            assert self.algo == ConvAlgo.MaskImplicitGemm, (
...
class SparseMaxPool1d(SparseMaxPool):
    def __init__(
        ...
        super(SparseMaxPool1d, self).__init__(
            1,
            kernel_size,
            stride,
            padding,
...
class SparseMaxPool4d(SparseMaxPool):
    def __init__(
        ...
        super(SparseMaxPool4d, self).__init__(
            4,
            kernel_size,
            stride,
            padding,
...
class SparseAvgPool1d(SparseAvgPool):
    ...
    def __init__(
        ...
        super(SparseAvgPool1d, self).__init__(
            1,
            kernel_size,
            stride,
            padding,
